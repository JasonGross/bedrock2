...
Proof. cbn. rewrite shamt_of_Z_small by (destruct width_cases as [-> | ->]; lia). rewrite eval_wmask'; trivial. Qed.
Lemma eval_wsize (m : mem) (l : locals) : Semantics.eval_expr m l wsize = Some (bits.of_Z width width).
Proof. cbn. rewrite shamt_of_Z_small by (destruct width_cases as [-> | ->]; lia). rewrite eval_wsize'; trivial. Qed.
End WithSemantics.
