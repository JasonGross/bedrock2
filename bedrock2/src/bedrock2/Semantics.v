...
Section operators.
  ...
  Definition interp_binop (bop : bopname) : word -> word -> word :=
    ...
    | bopname.sru => fun a b => Zmod.sru a (shamt b)
    | bopname.slu => fun a b => Zmod.slu a (shamt b)
    | bopname.srs => fun a b => Zmod.srs a (shamt b)
    | bopname.lts => fun a b =>
      if Z.ltb (Zmod.signed a) (Zmod.signed b) then Zmod.one else Zmod.zero
    | bopname.ltu => fun a b =>
      if Z.ltb (Zmod.unsigned a) (Zmod.unsigned b) then Zmod.one else Zmod.zero
    | bopname.eq => fun a b =>
