...
Require Import coqutil.Word.Bitwidth coqutil.Map.Interface. (* coercions word and rep *)
...
Section Scalars.
  ...
  Definition truncated_word sz addr (value: word) : mem -> Prop :=
    truncated_scalar sz addr (Zmod.unsigned value).
  ...
  Proof.
    ...
    erewrite (load_Z_of_sep width_pos); eauto; rewrite ?length_le_split; f_equal.
  ...
  Proof.
    cbv [truncated_scalar] in *; extract_ex1_and_emp_in_hyps.
    edestruct (uncurried_store_Z_of_sep width_pos); intuition eauto; rewrite ?length_le_split in *; trivial.
  ...
  Lemma load_one_of_sep addr value R m
    ...
    : Memory.load Syntax.access_size.one m addr = Some (bits.of_Z width (byte.unsigned value)).
  Proof.
  ...
  Lemma store_one_of_sep addr (oldvalue : byte) (value : word) R m (post:_->Prop)
    ...
    (Hpost : forall m, sep (scalar8 addr (byte.of_Z (Zmod.unsigned value))) R m -> post m)
  ...
  Proof.
    ...
    erewrite Zmod.add_0_r, get_sep by eauto.
    eexists; split; eauto; eapply Hpost.
    rewrite (map.of_list_word_singleton width_pos).
  ...
  Lemma scalar16_of_bytes a l (H : List.length l = 2%nat) :
    Lift1Prop.iff1 (array ptsto (bits.of_Z width 1) a l)
                   (scalar16 a (bits.of_Z width (LittleEndianList.le_combine l))).
  Proof.
    pose proof width_at_least_32.
    cbv [scalar16 truncated_word truncated_scalar].
    rewrite bits.unsigned_of_Z_small, split_le_combine'; trivial; cycle 1.
  ...
  Lemma scalar32_of_bytes a l (H : List.length l = 4%nat) :
    Lift1Prop.iff1 (array ptsto (bits.of_Z width 1) a l)
                   (scalar32 a (bits.of_Z width (LittleEndianList.le_combine l))).
  Proof.
    pose proof width_at_least_32.
  ...
  Lemma scalar_of_bytes a l (H : width = 8 * Z.of_nat (length l)) :
    Lift1Prop.iff1 (array ptsto (bits.of_Z width 1) a l)
                   (scalar a (bits.of_Z width (LittleEndianList.le_combine l))).
  Proof.
  ...
  Lemma store_four_of_sep addr (oldvalue : word32) (value : word) R m (post:_->Prop)
    (Hsep : sep (scalar32 addr oldvalue) R m)
    (Hpost : forall m, sep (scalar32 addr (bits.of_Z width (Zmod.unsigned value))) R m -> post m)
    : exists m1, Memory.store Syntax.access_size.four m addr value = Some m1 /\ post m1.
  Proof.
  *)
  ...
  Lemma array_store_of_sep (addr addr' : word) n (oldvalues : list word) (value : word) size sz R m (post:_->Prop)
    (_ : addr' = Zmod.add addr (bits.of_Z width ((Zmod.unsigned size) * (Z.of_nat n))))
    ...
    : exists m1, Memory.store sz m addr' value = Some m1 /\ post m1.
  Proof.
    rewrite <-(List.firstn_nth_skipn _ _ oldvalues (bits.of_Z width 0) H0) in Hsep.
  ...
  Lemma array_store_of_sep' (addr addr' : word) (oldvalues : list word) (value : word) size sz R m (post:_->Prop)
    (Hsep : sep (array (truncated_word sz) size addr oldvalues) R m)
    (_ : 0 < Zmod.unsigned size)
    (_ : let offset := Zmod.unsigned (Zmod.sub addr' addr) in
         (Z.modulo offset (Zmod.unsigned size) = 0) /\
         (let n := Z.to_nat (offset / Zmod.unsigned size) in (n < List.length oldvalues)%nat /\
  ...
  Lemma array_load_of_sep addr addr' n (values : list word) size sz R m
    ...
    Some (truncate_word sz (nth n values (bits.of_Z width 0))).
  Proof.
    rewrite <-(List.firstn_nth_skipn _ _ values (bits.of_Z width 0) H0) in Hsep.
    do 2 seprewrite_in (array_append (truncated_word sz) size) Hsep.
    seprewrite_in (array_cons (truncated_word sz) size) Hsep.
    seprewrite_in (array_nil (truncated_word sz) size) Hsep.
  ...
  Lemma array_load_of_sep' (addr addr': word) (values : list word) size sz R m
    ...
    : let offset := Zmod.unsigned (Zmod.sub addr' addr) in
      (Z.modulo offset (Zmod.unsigned size) = 0) ->
      (let n := Z.to_nat (offset / Zmod.unsigned size) in (n < List.length values)%nat ->
      Memory.load sz m addr' =
      Some (truncate_word sz (nth n values (bits.of_Z width 0)))).
  Proof.
    intros.
  ...
  Proof using BW.
    ...
    clear dependent mem; unfold byte.wrap; destruct width_cases; subst; lia.
  Qed.
  ...
  Proof using.
    pose proof Z.mod_pos_bound z 8.
    clear dependent mem; unfold byte.wrap; lia.
  Qed.
  ...
  Lemma wrap_byte_unsigned b:
    byte.unsigned b mod 2 ^ width = byte.unsigned b.
  Proof using BW.
    clear -BW.
    pose proof byte.unsigned_range b. rewrite Z.mod_small by (destruct width_cases as [-> | ->]; lia).
    reflexivity.
  Qed.
  ...
  Proof using. intros; clear dependent mem; unfold Memory.bytes_per, Memory.bytes_per_word; lia. Qed.
