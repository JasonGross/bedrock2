...
Ltac straightline :=
  ...
  | |- @get ?w ?L ?l ?x ?mc ?P =>
      ...
      change (get' w L l x mc P); cbv beta
  | |- load _ _ _ _ _ => cbv beta delta [load]
  | |- @MetricLoops.enforce ?width ?locals ?names ?values ?map =>
    let values := eval cbv in values in
    change (@MetricLoops.enforce width locals names values map);
...
Ltac show_program :=
  lazymatch goal with
  | |- @cmd ?width ?BW ?mem ?locals ?ext_spec ?E ?c ?F ?G ?H ?I =>
    let c' := eval cbv in c in
    change (@cmd width BW mem locals ext_spec E (fst (c, c')) F G H I)
  end.
