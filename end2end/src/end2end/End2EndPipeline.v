...
(* [KamiWord.wordW] at width 32 is [Naive.word 32] (by [eq_refl]), so the
   compiler's [riscv_ok] proof for Naive words applies as it is. *)
...
(* Kami's Word.v registers ZifyNat/ZifyN globally, so [lia] now translates
   every [nat] and [N] hypothesis it can see.  Inside the [end2end] proof below
   the context is large enough that this one arithmetic goal took 140 s and
   built a certificate deep enough to overflow the kernel stack at [Qed].
   Stated on its own it is a two-second [lia]. *)
